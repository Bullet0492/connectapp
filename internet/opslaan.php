...
if ($rij) {
    db()->prepare("UPDATE klant_internet SET provider=?, provider_anders=?, type=?, snelheid_down=?, snelheid_up=?, ip_adres=?, contract_datum=?, notities=? WHERE klant_id=?")
       ->execute([$velden['provider'], $velden['provider_anders'], $velden['type'], $velden['snelheid_down'], $velden['snelheid_up'], $velden['ip_adres'], $velden['contract_datum'], $velden['notities'], $klant_id]);
    ...
    db()->prepare("INSERT INTO klant_internet (klant_id, provider, provider_anders, type, snelheid_down, snelheid_up, ip_adres, contract_datum, notities) VALUES (?,?,?,?,?,?,?,?,?)")
       ->execute([$klant_id, $velden['provider'], $velden['provider_anders'], $velden['type'], $velden['snelheid_down'], $velden['snelheid_up'], $velden['ip_adres'], $velden['contract_datum'], $velden['notities']]);
