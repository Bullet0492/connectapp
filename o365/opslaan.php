...
$velden = [
    ...
    'aantal_licenties'   => (int)($_POST['aantal_licenties'] ?? 0),
    'mfa_actief'         => isset($_POST['mfa_actief']) ? 1 : 0,
    'conditional_access' => isset($_POST['conditional_access']) ? 1 : 0,
    'notities'           => trim($_POST['notities'] ?? ''),
];

if ($rij) {
    $ww_enc = $ww_nieuw !== '' ? encrypt_wachtwoord($ww_nieuw) : $rij['admin_wachtwoord_enc'];
    db()->prepare("UPDATE klant_o365 SET tenant_naam=?, tenant_id=?, admin_email=?, admin_wachtwoord_enc=?,
        licentie_type=?, aantal_licenties=?, mfa_actief=?, conditional_access=?, notities=?
        WHERE klant_id=?")
       ->execute([...$velden, $ww_enc, $klant_id]);
    ...
    db()->prepare("INSERT INTO klant_o365 (klant_id, tenant_naam, tenant_id, admin_email, admin_wachtwoord_enc,
        licentie_type, aantal_licenties, mfa_actief, conditional_access, notities)
        VALUES (?,?,?,?,?,?,?,?,?,?)")
       ->execute([$klant_id, ...$velden, $ww_enc]);
