...
if ($rij) {
    ...
    db()->prepare("UPDATE klant_o365 SET tenant_naam=?, admin_email=?, admin_wachtwoord_enc=?, notities=? WHERE klant_id=?")
       ->execute([$velden['tenant_naam'], $velden['admin_email'], $ww_enc, $velden['notities'], $klant_id]);
    ...
    db()->prepare("INSERT INTO klant_o365 (klant_id, tenant_naam, admin_email, admin_wachtwoord_enc, notities) VALUES (?,?,?,?,?)")
       ->execute([$klant_id, $velden['tenant_naam'], $velden['admin_email'], $ww_enc, $velden['notities']]);
