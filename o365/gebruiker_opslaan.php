...
if ($gebruiker_id) {
    ...
    db()->prepare('UPDATE klant_o365_gebruikers SET naam=?, email=?, wachtwoord_enc=?, rol=?, notities=? WHERE id=? AND klant_id=?')
       ->execute([$velden['naam'], $velden['email'], $ww_enc, $velden['rol'], $velden['notities'], $gebruiker_id, $klant_id]);
    ...
    db()->prepare('INSERT INTO klant_o365_gebruikers (klant_id, naam, email, wachtwoord_enc, rol, notities) VALUES (?,?,?,?,?,?)')
       ->execute([$klant_id, $velden['naam'], $velden['email'], $ww_enc, $velden['rol'], $velden['notities']]);
